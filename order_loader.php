...
use Sa\Sotbit\B2BShop\Helper;
use Sa\Sotbit\B2BShop\Controller;

class CSaleOrderLoaderCustom
{
	...
	function nodeHandler(CDataXML $dataXml, CXMLFileStream $fileStream)
	{
		...
		if($importer instanceof Sale\Exchange\ImportOneCBase)
		{
			...
			if(!$r->isSuccess())
			{
				...
				/*SA.SOTBIT.B2BSHOP*/
				// custom properties from 1c
				$app = Controller::get();
				$customProp1CName2OrderPropId = $app->getOption("import_1c_order_custom_props");
				$customBasketItemProp1CNames2Code = $app->getOption("import_1c_basket_item_custom_props");
				
				$orderCustomProps = [];
				$orderXMLTagRoot = $documentData[0]["#"];
				$orderId = intval($orderXMLTagRoot[GetMessage("CC_BSC1_ID")][0]["#"]);
				if($orderId AND is_numeric($orderXMLTagRoot[GetMessage("CC_BSC1_ID")][0]["#"])) {
					$order = \Bitrix\Sale\Order::load($orderId);
					$propertyCollection = $order->getPropertyCollection();
					foreach($customProp1CName2OrderPropId as $custom1CPropName => $orderPropId) {
						$custom1CPropValue = $orderXMLTagRoot[$custom1CPropName][0]["#"];
						if(strlen($custom1CPropValue)) {
							$propValue = $propertyCollection->getItemByOrderPropertyId($orderPropId);
							$propValue->setValue(date('d.m.Y', strtotime($custom1CPropValue)));
							$propValue->save();
						}
					}

					$basket = $order->getBasket();
					$basketItems1C = $orderXMLTagRoot[GetMessage("CC_BSC1_ITEMS")][0]["#"][GetMessage("CC_BSC1_ITEM")];
					if (is_array($basketItems1C)) {
						
						// 1C XML basket items loop
						foreach($basketItems1C as $basketItem1C) {
							
							$basketItem1C = $basketItem1C["#"];

							// custom properties loop
							foreach($customBasketItemProp1CNames2Code as $customBasketItemProp1CName => $customBasketItemProp1CCode) {
								
								$customBasketItemProp1CValue = $basketItem1C[$customBasketItemProp1CName][0]["#"];

								if(strlen($customBasketItemProp1CValue)) { // element in 1C XML has this property value
									
									$basketItem1CXmlId = $basketItem1C[GetMessage("CC_BSC1_ID")][0]["#"];

									// db basket items loop
									foreach ($basket as $basketItem) {
										
										if($basketItem->getField("PRODUCT_XML_ID") === $basketItem1CXmlId) { // element from db === element from 1C XML
											
											$basketPropertyCollection = $basketItem->getPropertyCollection();
											$basketPropertyCollection->setProperty([
												[
												   'NAME' => $customBasketItemProp1CName,
												   'CODE' => $customBasketItemProp1CCode,
												   'VALUE' => date('d.m.Y', strtotime($customBasketItemProp1CValue)),
												   'SORT' => 100,
												],
											]);
